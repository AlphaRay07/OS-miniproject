...
static void monitor_work_fn(struct work_struct *work)
{
    ...
    /* Workqueue context may sleep; mutex + RSS helper are safe here. */
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    list_for_each_entry_safe(curr, tmp, &monitor_list, list) {
        if (curr->pid == req.pid &&
            strncmp(curr->container_id, req.container_id, MONITOR_NAME_LEN) == 0) {
            list_del(&curr->list);
            kfree(curr);
            found = true;
            break;
